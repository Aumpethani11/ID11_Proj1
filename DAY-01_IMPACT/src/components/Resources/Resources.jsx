...
const resourceSigns = resourceSignOrder
  .map((signName) => SignImageData.find((sign) => sign.name === signName))
  .filter(Boolean);
...
const Resources = () => {
  ...
  const renderAlphabetGuide = () => (
    <div className="alphabet-guide">
      <h2>ASL Alphabet & Number Reference</h2>
      <div className="alphabet-grid">
        {resourceSigns.map((sign) => (
          <div key={sign.name} className="alphabet-item">
            <img src={sign.url} alt={`ASL sign for ${sign.name}`} />
          </div>
        ))}
      </div>
    </div>
